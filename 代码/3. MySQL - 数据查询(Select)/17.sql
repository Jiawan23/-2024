...
from
    ...
    -- 计算的这个差值一样时可以看作是连续的交易日
    from
		(select *,row_number() over(order by workday) rownum
        from
            -- 2022.02交易日总购买金额，workday表示第几个工作日
			(select pro_purchase_time t, sum(pro_quantity * f_amount) amount,
				@row :=datediff(pro_purchase_time, "2021-12-31")
                -- @声明一个变量
